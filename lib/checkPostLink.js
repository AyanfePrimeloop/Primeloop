 ...
 * `code` says WHY it failed, so callers can tell a definitely-bad link
 * (invalid / domain / redirect / not_found — fix it and try again) from a
 * maybe-temporary one (server_error / unreachable — worth a human look).
 */
export async function checkPostLink(postLink, platform) {
  ...
  try {
    ...
    return { ok: false, code: 'invalid', reason: 'This is not a valid web link.' };
  }
  ...
  if (!matchesDomain) {
    return { ok: false, code: 'domain', reason: `Link domain doesn't look like ${platform} (got ${url.hostname}).` };
  }
  ...
  try {
    ...
    if (!allowedDomains.some((d) => isAllowedHost(finalHost, d))) {
      return { ok: false, code: 'redirect', reason: `Link redirected away from ${platform} (ended up at ${finalHost}).` };
    }

    if (res.status === 404 || res.status === 410) {
      return { ok: false, code: 'not_found', reason: `The link returned a "not found" error (status ${res.status}).` };
    }
    if (res.status >= 500) {
      return { ok: false, code: 'server_error', reason: `The link's server returned an error (status ${res.status}) — may be temporary.` };
    }
    ...
    return { ok: false, code: 'unreachable', reason: `Couldn't reach the link at all (${err.message}).` };
  }
}
