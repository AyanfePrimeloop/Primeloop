...
async function runWeeklyPayout(supabaseAdmin, paystackSecretKey) {
  // Same variable the site reads for its wording (lib/payoutRules.js).
  const rawMin = Number(process.env.NEXT_PUBLIC_MIN_PAYOUT_NAIRA);
  const minPayout = Number.isFinite(rawMin) && rawMin >= 0 ? rawMin : 500;
  ...
  for (const [engagerId, group] of Object.entries(groups)) {
    ...
    // Below the minimum: leave everything unclaimed so it rolls over to the next
    // run. A bank transfer has a fixed fee that can be bigger than a tiny payout.
    if (expected < minPayout) {
      results.push({ engagerCode: engager.code, amount: expected, status: 'below_minimum' });
      continue;
    }
