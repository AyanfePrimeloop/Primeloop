...
export default function EngagerProfile() {
  ...
  return (
    ...
    <AppBar links={[
      ...
      { href: '/engager/profile', label: 'Profile', current: true },
      { href: '/choose-dashboard', label: 'Switch dashboard' },
    ]} />
    <div className="app" style={{ maxWidth: 560 }}>
      ...
      <div className="section">
        ...
        <div style={{ padding: 20 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 14, display: 'block', marginBottom: 5 }}>Full name</label>
            <input style={{ width: '100%' }} value={fullName} onChange={(e) => setFullName(e.target.value)} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 14, display: 'block', marginBottom: 5 }}>WhatsApp number</label>
          ...
          {result?.engager && <p style={{ color: 'var(--good)', fontSize: 14, marginTop: 10 }}>Saved.</p>}
        </div>
      </div>
      ...
      <div className="section">
        ...
        {platforms.length === 0 && (
          <p style={{ padding: 20, color: 'var(--ink-mute)', fontSize: 14 }}>
            No platforms registered yet — complete onboarding for a platform to add one.
          </p>
        )}
        {platforms.map((p) => (
          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid var(--line)', fontSize: 14 }}>
            <div>
              <div style={{ fontWeight: 500, textTransform: 'capitalize' }}>{p.platform}</div>
              <div style={{ fontSize: 13, color: 'var(--ink-mute)' }}>{p.profile_name} — {p.profile_link}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <a href={`/onboarding/${p.platform}`} className="btn" style={{ fontSize: 13, textDecoration: 'none' }}>
                Update page
              </a>
            </div>
          </div>
        ))}
        <p style={{ padding: '12px 20px', fontSize: 13, color: 'var(--ink-mute)' }}>
